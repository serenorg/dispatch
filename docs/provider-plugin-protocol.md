...
**Status:** The first reference implementations are `providers/seren-models` and `providers/seren-private-models` in `dispatch-seren-plugins`; they currently implement `capabilities`, `configure`, `health`, `complete`, and `shutdown`. `stream` and `cancel` are part of the protocol but return structured `unimplemented` errors in those reference plugins until streaming support is added.

Dispatch provider plugins are external executables that supply LLM inference to Dispatch hosts and couriers. They sit at the same layer as the in-process model backends that ship with `dispatch-core` and let couriers reach models served by third-party, first-party, or self-hosted endpoints without linking those backends into the Dispatch binary.

Dispatch uses JSON-RPC 2.0 messages framed as line-delimited JSON over stdio, matching the courier and channel plugin protocols. The protocol is designed for stream-first inference even though providers may also implement only non-streaming completion.
...
A provider plugin can implement:
...
The current Seren provider plugins forward `provider.complete` to OpenAI-compatible Seren publisher `/chat/completions` endpoints and do not yet implement streaming or cancellation.
...
Dispatch installs provider plugins through `dispatch extension install` and routes installed providers from the native courier's model-backend path. The `DISPATCH_BACKEND_<PROVIDER>` override resolves to the installed plugin when one is registered for that provider name. Streaming and routing through other courier backends are still under development.
...
Dispatch supports protocol version `1`. The `kind` field is optional for local manifests so conventional filenames and catalog metadata can infer the plugin kind, but when present it must be `"provider"`.

## Requests

Every host call is sent as a JSON-RPC request. The `method` identifies the provider operation and `params` contains the Dispatch request envelope. Request params always include `protocol_version` and `kind`.
...
`provider.capabilities` returns the provider's feature declaration. Callers can use this to decide which models and features are safe to route through this provider. The native courier's initial provider bridge resolves providers by configured provider name and registry lookup; capability-gated routing is expected to become more important as provider plugins replace in-process OpenAI, Anthropic, Gemini, Claude CLI, and Codex backends.
...
```json
{
  ...
  "capabilities": {
    ...
    "models": [
      {
        "id": "*",
        "display_name": "Seren Models (wildcard)"
      }
    ],
...
This example mirrors the current `seren-models` response. Providers may list models statically, or return a wildcard model entry (`"id": "*"`) and resolve specific model IDs at `complete`/`stream` time. `seren-models` and `seren-private-models` use the wildcard form.

Couriers that match a model against a provider use whichever model IDs capabilities declared.

## Configuration

`provider.configure` validates credentials, endpoint URLs, and optional defaults. Config values are passed through from Dispatch's runtime config or from the invoking courier. The config object is plugin-specific.

The current Seren provider plugins accept:

- `api_origin` - optional Seren API origin, defaulting to `https://api.serendb.com`
- `api_key` - Seren API key with access to the relevant publisher

If `config` is null or an empty object, the Seren provider plugins resolve the same values from `SEREN_API_ORIGIN` and `SEREN_API_KEY`. `SEREN_API_KEY` is required unless `api_key` is supplied in the request config.
...
`account_id` and `default_model` are optional. The current Seren provider plugins return neither and report the resolved publisher URL through `extensions.base_url`.
...
`provider.complete`, `provider.stream`, and `provider.cancel` carry their substantive payloads under `params` or `id` while the message shapes continue to iterate.
...
`provider.stream` is stream-first. The plugin emits zero or more JSON-RPC notifications with method `provider.event`, followed by exactly one terminal success response whose `result.kind` is `completion`. The current Seren provider plugins return `unimplemented` for `provider.stream`.
